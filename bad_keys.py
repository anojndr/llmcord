            ...
            try:
                ...
                if "stream not found" in error_str or "Hrana" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logging.exception(f"Failed to reconnect to Turso after {max_retries} attempts: {e}")
                        raise
                else:
                    raise
    return wrapper
...
class KeyRotator:
    ...
    def __init__(self, provider: str, all_keys: list[str], max_retries_multiplier: int = 2):
    ...
    def _init_good_keys(self):
        ...
        if self._good_keys is None:
            ...
            self._good_keys = self._db.get_good_keys_synced(self.provider, self.all_keys)

            # If all keys are bad, reset and try again with all keys
            if not self._good_keys:
                logging.warning(f"All API keys for '{self.provider}' are marked as bad. Resetting...")
                self._db.reset_provider_keys_synced(self.provider)
                self._good_keys = self.all_keys.copy()

    def get_keys(self):
        """Generator that yields API keys to try, handling rotation and retries.
    ...
    async def get_keys_async(self):
        """Async generator that yields API keys to try, handling rotation and retries.
        Same as get_keys() but for async contexts.
        """
        for key in self.get_keys():
            yield key

    def mark_current_bad(self, error_message: str = None):
        ...
        logging.warning(f"API key failed for '{self.provider}' (attempt {self._attempt_count}): {error_msg}")
        ...
        if not self._good_keys:
            if self._attempt_count >= len(self.all_keys):
                logging.warning(f"All keys exhausted for '{self.provider}'. Resetting synced keys for retry...")
                self._db.reset_provider_keys_synced(self.provider)
                self._good_keys = self.all_keys.copy()
